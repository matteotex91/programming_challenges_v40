...
def cost_gradient_W_D(W1, D1, W2, D2, x_calc_arr, y_exp_arr):
    ...
    for x_calc, y_exp in tqdm(zip(x_calc_arr, y_exp_arr)):
...
W1 = 1 - 2 * np.random.random(shape1)  # generate random weights
D1 = 1 - 2 * np.random.random(size_2)  # generate random biases
W2 = 1 - 2 * np.random.random(shape2)  # generate random weights
D2 = 1 - 2 * np.random.random(size_3)  # generate random biases
...
for i in range(iterations):
    print(f"iteration {i+1} over {iterations}")
    (
        g_W1,
        g_D1,
        g_W2,
    ...
    if (
        np.sum(np.isnan(W1)) > 0
        or np.sum(np.isnan(D1)) > 0
        or np.sum(np.isnan(W2)) > 0
        or np.sum(np.isnan(D2)) > 0
    ):
        print("NAN values")
