...
def replot_transform():
    ...
    for i in range(N):
        good_indexes = np.where(np.abs(iterMat) <= 2)
        iterMat[good_indexes] = np.power(iterMat[good_indexes], 2) + mat[good_indexes]
    abs_mat = np.abs(iterMat)
    abs_mat = abs_mat - np.min(abs_mat)
    abs_mat = abs_mat / np.max(abs_mat)
    ax.clear()
    ax.pcolormesh(abs_mat)
    # ax.scatter(
    ...
    def on_release(event: KeyEvent):
        ...
        match event.key:
            case "right":
                # re0 += add_const
                # re1 += add_const
                re0 += shift_coefficient * (re1 - re0)
                re1 += shift_coefficient * (re1 - re0)
            case "up":
                cx0 += shift_coefficient * (cx1 - cx0)
                cx1 += shift_coefficient * (cx1 - cx0)
                # cx0 += add_const
                # cx1 += add_const
            case "down":
                cx0 -= shift_coefficient * (cx1 - cx0)
